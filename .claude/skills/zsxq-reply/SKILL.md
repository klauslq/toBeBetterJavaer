...
- **深度**：不求短。多问题、路线规划类的提问值得写到 1000 字左右；单点决策类问题几百字讲透即可，字数匹配问题体量，不注水。有术（具体怎么做，给可直接抄的示例，如简历句式带量化数据占位）有道（背后的判断依据）。合格标准：球友读完知道下一步该做什么、面试该说什么（详见 `references/exemplars.md`）
