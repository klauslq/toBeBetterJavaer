...
不要只给结论。正确的节奏是：抛出问题 → 展示一手证据（源码/数据/官方原文）→ 用自己的话解读证据的含义。读者看到证据才会信服你的结论。

```
❌ "skill 的 description 有字数限制，超出会被截断。"
✅ "源码里写得明明白白：
    export const MAX_LISTING_DESC_CHARS = 250
   超出 250 字符的部分直接被砍掉，换成一个省略号。你在第 300 个字符处写的精妙触发条件，模型从头到尾就没见过。"
```
...
文章的核心观点必须有一手证据支撑，一手证据包括：
- 源码片段（标注文件路径和关键行）
- 官方博客/文档的原文引用
- 实测数据（自己跑出来的结果）
- GitHub issue/PR 中的讨论原文

禁止用"据说""业内普遍认为""有人发现"这类无来源表述支撑核心论点。

##### 正文节奏

每个章节遵循"**悬念 → 证据 → 解读 → 实操建议**"四拍：

1. **悬念**：用一个具体问题或反直觉的现象开场（"为什么装了 50 个 skill，一个都不触发？"）
2. **证据**：展示源码/数据/官方原文，让读者亲眼看到事实
3. **解读**：用自己的话解释证据的含义，点明"这对你意味着什么"
4. **实操建议**：一句话告诉读者该怎么做（加粗或独立成段，方便扫读）

##### 代码片段规范

- 展示源码时标注来源：文件路径或仓库名 + 相对路径
- 只截取和观点直接相关的片段（5-15 行），不贴大段无关代码
- 代码前后各一句话：前面说"这段代码在做什么"，后面说"这意味着什么"

##### 章节衔接

章节之间用"这就引出了下一个问题"或"明白了这个机制，X 的答案就浮出来了"做自然过渡，让读者感受到问题链的递进关系，而不是孤立的知识点堆砌。
