...
> 仅在步骤 3 选择「面试对话类」时生效。完整规范——对话体框架（老王出场方式、场景描写密度、回答起手交替）、对话体与双轨表达的边界、技术细节尺度（禁自定义类名）、写法示例——全部在 `references/interview-style.md`，撰写前通读，不在此重复。
...
素材 `sucai.md` 里有现成标题就直接用，回填 frontmatter 的 title 和 shortTitle。没有现成标题时（不调用 title-generator Skill）：

1. 跑 `./scripts/read_titles.py` 拿到公众号高阅读标题清单（按阅读人数倒序，标注字数）
2. 结合用户给的标题关键字/意向标题和文章核心信息，从清单头部（阅读量高的优先）挑出句式适配的模板
3. **严格复刻模板句式，只替换关键词**，不改句子骨架和标点结构；新标题与模板的字数误差 ≤ 2（含标点和空格，用 `python3 -c "print(len('标题'))"` 校验）
4. 给用户 5 个候选，每个标注套用的模板和该模板的阅读数，用户选定后回填 frontmatter
